...
import { getNeighborhoods, createNeighborhood, deleteNeighborhood } from '../../../services/NeighborhoodsService';
...
export default function NeighborhoodsAdmin() {
  ...
  const onCreate = async (e) => {
    ...
    setCreating(true);
    try {
      await createNeighborhood({ name: newName.trim() });
      await load();
      setNewName('');
    } catch (err) {
  ...
  return (
    <section className="admin-page">
      ...
      <form onSubmit={onCreate} style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
        <input placeholder="Nombre del barrio" value={newName} onChange={(e) => setNewName(e.target.value)} className="input" />
        <button type="submit" className="btn" disabled={creating}>{creating ? 'Creando...' : 'Crear'}</button>
      </form>

      {loading && <p>Cargando...</p>}
      {error && <p className="text-danger">Error al cargar barrios</p>}
      {!loading && !error && (
        <table className="admin-table">
          ...
          <tbody>
            {items && items.length ? (
              ...
              <tr>
                <td colSpan={3}>No hay barrios</td>
              </tr>
            )}
          </tbody>
        </table>
